...
locals {
  ...
  sa_name     = var.service_account_name    # Set the service account name
  zone        = var.zone_name               # Set the zone name
...
resource "yandex_vpc_security_group" "k8s-main-sg" {
  ...
  network_id  = yandex_vpc_network.k8s-network.id
    ...
    ingress {
    description    = "The rule allows connection to node cluster by SSH from the Internet"
    protocol       = "TCP"
